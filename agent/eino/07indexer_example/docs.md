...
存储向量库时使用，把文档和文档的向量存储到后端存储系统。需要向量型数据库，不是普通的 MySQL 那种关系型数据库。

Demo 使用 Milvus 原生支持，通过 Docker 启动。
...
可以看 Milvus 的官方文档 <https://milvus.io/docs/zh/install-overview.md>，但是感觉中文是机翻的。要写一个 Go 客户端去连接并存储数据。
...
脚本已经下载到该目录里了，直接启动；如果启动报错，就在前面加 `./` 前缀。

诶诶可恶的powershell（我要重装电脑）

访问 `http://127.0.0.1:9091/webui/` 会有一个 Web 页面，相当于一个管理端，但是看不到更具体的数据。查看官方文档后发现，Attu 可以作为管理 Milvus 的 Web App。

Attu 的 Release 页面也支持 Docker 启动。
...
然后在写代码的时候发现官方文档的代码示例是错的……

这里注意，Attu 连接 Milvus 的时候**不要填** `localhost:19530` 或 `127.0.0.1:19530`，因为 Attu 前端在浏览器里运行，浏览器会把 `localhost` 解析成它自己，而不是你的电脑。
